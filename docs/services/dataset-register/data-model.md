...
| [`schema:dateRead`](https://schema.org/dateRead)             | UTC datetime when the URL was last read by the application. Both the crawler and a re-registration through `POST /datasets` update it, because both fetch and re-store the descriptions.                                                                                                                                                                                         |
| `nde:dateCrawled` (`<https://def.nde.nl/registration#dateCrawled>`) | `xsd:dateTime` – when the crawler last crawled the URL, which is also when its distributions were last link-checked. Distinct from `schema:dateRead`: a re-registration reads the URL but probes nothing, so it does not advance this value. The crawler picks up every registration whose `nde:dateCrawled` is older than the TTL, or absent. |
