...
1. **Select** valid dataset descriptions with at least one RDF distribution the pipeline can read *in full* from the [Dataset Register](../dataset-register/index.md): a SPARQL endpoint, or a dump with an RDF media type. A distribution that declares itself an API we cannot read in full – through `dct:conformsTo`, for example an LDES, a IIIF Change Discovery or a Linked Art Search endpoint – is skipped, even when it honestly serves `application/ld+json`: parsing it would yield only its entry page and publish that as the dataset’s summary. A `dct:conformsTo` that names a *content model* rather than an access protocol (EDM, SCHEMA-AP-NDE, Persons in Context) does not disqualify a dump.
...
7. **Reconcile** the cache with the register: a dataset that has since been removed from the register or whose registration source has gone (became unreachable) is no longer rewritten, so its file would linger as a stale “ghost”. After writing, the pipeline deletes every file whose dataset URI is no longer present-and-not-gone in the register. Datasets merely skipped this run (no RDF distribution, an unreachable endpoint, or a description that failed validation) are kept, and an empty result from the register prunes nothing – so a register outage can never empty the cache. A dataset that offers *only* APIs the pipeline cannot read in full is the exception: that is a permanent, declared property of the registration rather than a transient failure, so any earlier – and by construction misleading – summary is pruned.

Datasets without a readable RDF distribution are skipped and get no summary at all, rather than a partial one; invalid distributions emit a `schema:error` triple instead of a summary, so consumers can still see *which* distributions are unreachable.
