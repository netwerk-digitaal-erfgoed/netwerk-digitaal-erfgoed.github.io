...
| 3    | Normalisation *(normalisatie van gegevens)* | No dedicated package – realised elsewhere: literal normalisation (dates, names, capitalisation) in [Transformation](../patterns.md#sparql-native-transformation) via CONSTRUCT and [adapters](../patterns.md#adapters); search-time folding/stemming is [engine-dependent](#normalization-is-engine-dependent) | –                                                                                                                       | **SCHEMA-AP-NDE-aware normalisation** (`@ndes/*`): coping with the real-world deviations from the profile that the [DKG](../../services/dataset-knowledge-graph/) measures – e.g. collapsing a `schema:dateCreated` period or range to a single sortable, facetable date for search. Cf. [`stack#10`](https://github.com/netwerk-digitaal-erfgoed/stack/issues/10) |
