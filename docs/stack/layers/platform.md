...
|------|----------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Data Layer** |  |  |  |  |
| 1    | Ingest sources *(opname van de bronnen)* | [`dataset-registry-client`](https://github.com/ldelements/lde/tree/main/packages/dataset-registry-client) (discover + `SearchCriteria` selection), [`distribution-downloader`](https://github.com/ldelements/lde/tree/main/packages/distribution-downloader), [`distribution-probe`](https://github.com/ldelements/lde/tree/main/packages/distribution-probe), [`distribution-monitor`](https://github.com/ldelements/lde/tree/main/packages/distribution-monitor), [`sparql-importer`](https://github.com/ldelements/lde/tree/main/packages/sparql-importer), [`sparql-qlever`](https://github.com/ldelements/lde/tree/main/packages/sparql-qlever), [`pipeline-void`](https://github.com/ldelements/lde/tree/main/packages/pipeline-void) (VoID analysis for source selection) | [NDE Datasetregister](../../services/dataset-register); [NDE Dataset Knowledge Graph](../../services/dataset-knowledge-graph) (data-driven source selection) | Pipelines read distribution health from the [Register’s health graph](../../services/dataset-register/data-model.md#distribution-health) rather than re-probing. LDES Executor. Mature “Knowledge Graph voor Datasets” network service; advisor service surfacing DKG analyses to dienstplatformen |
| 2    | Select metadata *(selectie van metadata)* | [`@lde/pipeline`](https://github.com/ldelements/lde/tree/main/packages/pipeline): the Select stage plus parameterised SPARQL CONSTRUCT extraction ([SPARQL-native Transformation](../patterns.md#sparql-native-transformation)) slice the ingested data to the relevant subset by adapting selectors and CONSTRUCT queries | – | Per the report, primarily served source-side: bronhouders publish “op maat” datasets (e.g. all objects carrying a WO2-Thesaurus term), discoverable via the [Datasetregister](../../services/dataset-register) |
| 3    | Normalisation *(normalisatie van gegevens)* | No dedicated package – realised elsewhere: literal normalisation (dates, names, capitalisation) in [Transformation](../patterns.md#sparql-native-transformation) via CONSTRUCT and [adapters](../patterns.md#adapters); search-time folding/stemming is [engine-dependent](#normalization-is-engine-dependent) | –                                                                                                                       | **SCHEMA-AP-NDE-aware normalisation** (`@ndes/*`): coping with the real-world deviations from the profile that the [DKG](../../services/dataset-knowledge-graph/) measures – e.g. collapsing a `schema:dateCreated` period or range to a single sortable, facetable date for search. Tracked in [`stack#10`](https://github.com/netwerk-digitaal-erfgoed/stack/issues/10) |
| 4a   | Transformation (technical) *(transformatie naar een geharmoniseerd model)* | [`distribution-downloader`](https://github.com/ldelements/lde/tree/main/packages/distribution-downloader), [`sparql-importer`](https://github.com/ldelements/lde/tree/main/packages/sparql-importer), [`sparql-qlever`](https://github.com/ldelements/lde/tree/main/packages/sparql-qlever). Non-RDF sources (CSV, JSON, XML) are transformed by the same [SPARQL-native Transformation](../patterns.md#sparql-native-transformation) via [SPARQL Anything](https://sparql-anything.cc), so they need no separate toolchain. | –                                                                                                                       | SHACL-driven transformation generator – the realisation of [`ldelements/lde#325`](https://github.com/ldelements/lde/issues/325). Network-level facility offering non-RDF-only sources as RDF distributions, so platforms need not each re-convert (the report’s *automatische transformatie naar linked data*). |
| 4b   | Transformation (semantic) *(transformatie naar een geharmoniseerd model)* | –                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | [SCHEMA-AP-NDE-first](../patterns.md#schema-ap-nde-first) | domain-profile adapters (Linked Art / CIDOC-CRM, RiC-O, RDA, EDM), each a SPARQL CONSTRUCT ([SPARQL-native Transformation](../patterns.md#sparql-native-transformation)) executed automatically in the pipeline; auto-generating the mapping from SHACL shapes is [`lde#325`](https://github.com/ldelements/lde/issues/325) |
| 5    | Enrich *(inwinnen van verrijkingen)* | –                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | [NDE Network of Terms](../../services/network-of-terms)                                                                 | `@ndes/pipeline-tn-enrich` (calls NoT); NDE “Knowledge Graph voor Termen” network service (term ↔ term, [report function 5](#function-mapping), substrate C); **Term Backlink Graph** (term ↔ record, derived from substrate B). Enrichments produced here – term links, annotations ([Web Annotation Data Model](https://www.w3.org/TR/annotation-model/)) – are published back to the network via [Enrichments-as-Datasets](../patterns.md#enrichments-as-datasets), reusable as dataset or LDES stream. |
| 6    | Build knowledge graph *(opbouwen van een knowledge graph)* | [`sparql-qlever`](https://github.com/ldelements/lde/tree/main/packages/sparql-qlever), [`sparql-server`](https://github.com/ldelements/lde/tree/main/packages/sparql-server), [`local-sparql-endpoint`](https://github.com/ldelements/lde/tree/main/packages/local-sparql-endpoint), [`pipeline`](https://github.com/ldelements/lde/tree/main/packages/pipeline)                                                                                                                      | –                                                                                                                       | `@ndes/pipeline-terms-kg-project`; `netwerk-digitaal-erfgoed/term-kg` (Term Backlink Graph deployment repo); `netwerk-digitaal-erfgoed/stack` (Nx monorepo for `@ndes/*` packages)                                                                                                                                          |
| 7    | Build search index *(opbouwen van een zoekindex)* | [`pipeline-shacl-validator`](https://github.com/ldelements/lde/tree/main/packages/pipeline-shacl-validator), [`pipeline-shacl-sampler`](https://github.com/ldelements/lde/tree/main/packages/pipeline-shacl-sampler), [`pipeline-void`](https://github.com/ldelements/lde/tree/main/packages/pipeline-void)                                                                                                                                                                                                                                                                                                | –                                                                                                                       | `@lde/pipeline-construct-extract`, `@lde/pipeline-frame-reshape`, `@lde/pipeline-typesense-writer` (load + finalize as one transaction-aware `Writer`); `@ndes/schema-profile-search-config` (object search), `@ndes/dcat-ap-nl-search-config` (catalog search over dataset descriptions); Search pipeline                                                                                                                   |
| 8    | Expose Data Layer *(beschikbaar stellen)* | [`fastify-rdf`](https://github.com/ldelements/lde/tree/main/packages/fastify-rdf)                                                                                                                                                                                                                                                                                                                                                                                                                                   | –                                                                                                                       | `@lde/search-api-graphql`, `@lde/search-api-rest` (generated surfaces over the `@lde/search` core + `@lde/search-typesense` adapter; see [ADRs](https://github.com/ldelements/lde/tree/main/docs/decisions)); `@lde/ldes-static-writer`                                                                                                                                                                                                                                |
| **Presentation Layer** |  |  |  |  |
| 9    | Retrieve metadata *(ophalen van metadata)* |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | –                                                                                                                       | Per-deployment presentation-layer adapters fetching the Data Layer’s GraphQL – directly from the browser ([Projection mode](#two-consumption-modes)’s single round-trip), or via a server-side rendering layer for SEO or multi-backend fan-in                                                                                                                                                                                                                          |
| 10   | Present metadata *(presenteren van metadata)* | (out of scope)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | –                                                                                                                       | `@ndes/heritage-ui-components`                                                                                                                                                                                                                                                                                  |
> 
...
- **Gaps (–) are not all equal.** [Function 3](#function-mapping) has no dedicated component because normalisation is *primarily* a Data Provider concern – the report puts it at source (“idealiter vindt de normalisatie van gegevens direct bij de bron plaats”). But it is not only a source concern: some normalisation is always potentially needed in the Data Layer, and it is realised across other steps rather than in a step-3 component. **Literal-level** normalisation (date fields, capitalisation, person-name rendering) happens in the [Transform stage](#function-mapping) via CONSTRUCT, with bespoke cases at a [Ports & Adapters](../patterns.md#adapters) boundary; **search-time** normalisation (diacritic folding, stemming) is [engine-dependent](#normalization-is-engine-dependent); and **abstraction-level** normalisation – flattening an archive inventory that describes a box against a museum record that describes an object – requires interpretation the source never makes, so it is semantic [Transformation](#function-mapping) and [enrichment](#function-mapping). [Function 10](#function-mapping) is empty because presentation components live in the application layer the Stack supports rather than constituting it.
...
- **Knowledge Graph voor Termen** *(proposed, term ↔ term, substrate C)* – planned network service per [report function 5](#function-mapping). Aggregates the **declared** equivalence relations across terminology sources: a producer pipeline crawls substrate C (the terminology sources, via the [Network of Terms](../../services/network-of-terms/)) and harvests the cross-vocabulary mapping predicates the sources already publish – `skos:exactMatch`, `skos:closeMatch`, `owl:sameAs` – materialising the equivalence graph. A query API on top returns a term's equivalents, so a Wikidata URI resolves to its NTA counterpart wherever the sources declare the link (DERA's [Alignment](https://dera.netwerkdigitaalerfgoed.nl/index.php/Alignment)). Scope for now is these **explicit** relations; deriving matches by label where no mapping is declared is a separate, later concern.
