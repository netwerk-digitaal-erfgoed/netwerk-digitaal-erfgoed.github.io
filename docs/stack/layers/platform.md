...
The Search APIs face the Presentation Layer and expose the search index produced by the [Search Pipeline](#search-pipeline). Both the GraphQL API (schema and Mercurius resolvers) and the REST API (OpenAPI spec and Fastify route handlers) are generated from the same SHACL + `search:` source of truth that drives the pipeline – SDL generation is part of the projection step, not a separate hand-written contract. Hand-writing either spec or its implementation is the bug, not the feature.
