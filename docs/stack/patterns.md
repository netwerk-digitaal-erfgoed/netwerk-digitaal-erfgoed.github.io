...
- **KG triplestore.** [Knowledge graphs](layers/platform.md#data-layer) build into any SPARQL-compliant store; the port is *load these triples, serve SPARQL*. Adapters range by scale: [QLever](https://github.com/ad-freiburg/qlever) is the Stack default for large bulk-loaded KGs (its read-only-after-load lifecycle fits [blue/green rebuild](#bluegreen-rebuild)); GraphDB and Jena Fuseki are heavier server alternatives; [Oxigraph](https://github.com/oxigraph/oxigraph) runs embedded and **in-memory**; and [`@lde/local-sparql-endpoint`](https://github.com/ldelements/lde/tree/main/packages/local-sparql-endpoint) is the simplest form – an in-process endpoint with no separate server to operate. **HDT** fits as a compact, read-only backing store or as a downloadable KG distribution. A Service Platform stands up a SPARQL endpoint only when it needs graph queries – the query contract is SPARQL, but it need not be public or standalone, and a search-only platform skips the KG entirely and keeps just its [search index](layers/platform.md#search-pipeline).
