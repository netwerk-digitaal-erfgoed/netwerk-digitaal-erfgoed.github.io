...
- **KG triplestore.** [Knowledge graphs](layers/platform.md#data-layer) build into any SPARQL-compliant store; the port is *load these triples, serve SPARQL*. Adapters range by scale: [QLever](https://github.com/ad-freiburg/qlever) is the Stack default for large bulk-loaded KGs (its read-only-after-load lifecycle fits [blue/green rebuild](#bluegreen-rebuild)); GraphDB and Jena Fuseki are heavier separately-operated servers; [Oxigraph](https://github.com/oxigraph/oxigraph) runs **embedded and in-memory**, as a library inside the app with no separate server; and at the smallest end a [Comunica](https://comunica.dev/) engine queries an RDF **file directly**, with no triplestore and no load step (`comunica-sparql-file-http`, wrapped by [`@lde/local-sparql-endpoint`](https://github.com/ldelements/lde/tree/main/packages/local-sparql-endpoint) as a test/dev helper today). **HDT** fits as a compact, read-only backing store or as a downloadable KG distribution. A Service Platform stands up a SPARQL endpoint only when it needs graph queries – the query contract is SPARQL, but the store can be embedded/in-memory (Oxigraph) or a file queried directly (Comunica) rather than a separately-operated server, and a search-only platform skips the KG entirely and keeps just its [search index](layers/platform.md#search-pipeline).
